...
@interface LixilShareBubbles()
@end

@implementation LixilShareBubbles
...
-(void)productShowTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_ProductShow];
}

// 套件合计
-(void)collectionTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_Colletion];
}

// 工厂介绍
-(void)factioryIntrodutionTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_FactioryIntrodution];
}

// 领先技术
-(void)leadingTechTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_LeadingTech];
}

// 品牌理念
-(void)brandConceptTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_BrandConcept];
}

// 销售网点
-(void)salesOutletsTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_SalesOutlets];
}

// 3D空间
-(void)lixilWooden3DroomTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_3DRoom];
}

// 品牌历史
-(void)brandHistoryTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_BrandHistory];
}

// 工程案例
-(void)philosophyTapped {
    [self shareButtonTappedWithType:LixilMenuItemType_Philosophy];
}


-(void)shareButtonTappedWithType:(LixilShareBubbleType)buttonType {
    //  [self hide];
    if([self.delegate respondsToSelector:@selector(lixilShareBubbles:tappedBubbleWithType:)]) {
        [self.delegate lixilShareBubbles:self tappedBubbleWithType:buttonType];
    }
}
...
-(void)show
{
    if(!self.isAnimating)
    {
        ...
        UIButton *item1Bubble = [self shareButtonWithIcon:@"lixil_home_showproducts" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        ...
        UIButton *item2Bubble = [self shareButtonWithIcon:@"lixil_home_suitecollection" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        ...
        UIButton *item3Bubble = [self shareButtonWithIcon:@"lixil_home_factioryintrodution" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item3Bubble addTarget:self action:@selector(factioryIntrodutionTapped) forControlEvents:UIControlEventTouchUpInside];
        // [item3Bubble setFrame:MenuItem3_Frame];
        [self addSubview:item3Bubble];
        [bubbles addObject:item3Bubble];
        
        UIButton *item4Bubble = [self shareButtonWithIcon:@"lixil_home_leadingtech" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        ...
        UIButton *item5Bubble = [self shareButtonWithIcon:@"lixil_home_brandconcept" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item5Bubble addTarget:self action:@selector(brandConceptTapped) forControlEvents:UIControlEventTouchUpInside];
        // [item5Bubble setFrame:MenuItem5_Frame];
        [self addSubview:item5Bubble];
        [bubbles addObject:item5Bubble];
        
        UIButton *item6Bubble = [self shareButtonWithIcon:@"lixil_home_salesoutlets" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item6Bubble addTarget:self action:@selector(salesOutletsTapped) forControlEvents:UIControlEventTouchUpInside];
        // [item6Bubble setFrame:MenuItem6_Frame];
        [self addSubview:item6Bubble];
        [bubbles addObject:item6Bubble];
        
        UIButton *item7Bubble = [self shareButtonWithIcon:@"lixil_home_3droom" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item7Bubble addTarget:self action:@selector(lixilWooden3DroomTapped) forControlEvents:UIControlEventTouchUpInside];
        // [item7Bubble setFrame:MenuItem7_Frame];
        [self addSubview:item7Bubble];
        [bubbles addObject:item7Bubble];
        
        UIButton *item8Bubble = [self shareButtonWithIcon:@"lixil_home_brandhistory" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item8Bubble addTarget:self action:@selector(brandHistoryTapped) forControlEvents:UIControlEventTouchUpInside];
        // [item8Bubble setFrame:MenuItem8_Frame];
        [self addSubview:item8Bubble];
        [bubbles addObject:item8Bubble];
        
        UIButton *item9Bubble = [self shareButtonWithIcon:@"lixil_home_philosophy" andBackgroundColorRGB:self.mailBackgroundColorRGB];
        [item9Bubble addTarget:self action:@selector(philosophyTapped) forControlEvents:UIControlEventTouchUpInside];
