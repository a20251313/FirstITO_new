...
@interface InaxBaseViewController ()<UITextFieldDelegate>
{
...
@implementation InaxBaseViewController
...
#pragma mark -搜索相关-
    ...
    // 2,3,4 控件的父视图
    UIView *serchModuleView  = [[UIView alloc] initWithFrame:CGRectMake(880, 25, 327,40)];
    ...
    [navView addSubview:serchModuleView];
    
#pragma mark -搜索相关-
    
    UIImage *settingImage = [UIImage imageNamed:@"inax_nav_setting"];
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    {
        if ([self isKindOfClass:[InaxSearchViewController class]])
        {
            InaxSearchViewController *isvc = (InaxSearchViewController *)self;
            isvc.searchKeyWord = searchText;
            [isvc reloadData];
        }
        else
        {
            InaxSearchViewController *isvc = [[InaxSearchViewController alloc] initWithNibName:@"InaxSearchViewController" bundle:nil];
            isvc.searchKeyWord = searchText;
...
{
    [InaxRightSettingBar showSettingBarInVC:self];
}
