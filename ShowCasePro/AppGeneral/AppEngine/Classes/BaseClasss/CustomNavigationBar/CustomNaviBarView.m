...
{
    return Rect(926.0f, 14.0f, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
}

+ (CGSize)barBtnSize
{
    return Size(78.0f, 80.0f);
}

+ (CGSize)barSize
{
    return Size(1024.0f, 85.0f);
}
...
- (void)initUI
{
    ...
    // 默认左侧显示返回按钮
    _btnBack = [[self class] createImgNaviBarBtnByImgNormal:@"lixilGroup_back" imgHighlight:@"lixilGroup_back" target:self action:@selector(btnBack:)];
    
    _btnRight = [[self class] createImgNaviBarBtnByImgNormal:@"lixilGroup_setting" imgHighlight:@"lixilGroup_setting" target:self action:@selector(btnRightEvent:)];
    ...
    _imgViewBg.image = [[UIImage imageNamed:@"lixilGroup_nav_bar"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
...
- (void)btnBack:(id)sender
{
    ...
    {
        [self.m_viewCtrlParent.navigationController popViewControllerAnimated:NO];
    }else{APP_ASSERT_STOP}
}


- (void)btnRightEvent:(id)sender
{
    //[RightSettingBar settingBarWithSuperViewController:self];
}
