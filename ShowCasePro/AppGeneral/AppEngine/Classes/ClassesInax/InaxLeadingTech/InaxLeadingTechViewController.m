...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if(_ContentArray!=nil)
    {
        ...
        if([pageString equals:@"inax_lt_regio"]||[pageString equals:@"inax_lt_naiyi"])
        {
            CGPoint offset = _scrollContent.contentOffset;
        ...
        if([pageString equals:@"inax_lt_oufute"]||[pageString equals:@"inax_lt_suolaisi"]||[pageString equals:@"inax_lt_zidonglongtou"]||[pageString equals:@"inax_lt_shumo"]||[pageString equals:@"inax_lt_yingyashi"]||[pageString equals:@"inax_lt_anmoyugang"])
        {
            CGPoint offset = _scrollContent.contentOffset;
            pageImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"inax_cheng_lt_page%d", (int)offset.y/683+1]];
        }
...
- (void) scrollMethod
{
    if(!isDetailView)
  {
    ...
    if(section0_contentOffSet.x >= 1024/6 * 21 * 35)
    {
...
- (void)viewDidLoad
{
    ...
    _imgArray = [NSMutableArray arrayWithCapacity:21];
    _desImgArray = [NSMutableArray arrayWithCapacity:21];
    for(int i= 1;i<22;i++)
    {
...
- (InaxLeadingTechCell  *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UIImage  *imgTemp  = (UIImage*)[_imgArray objectAtIndex:indexPath.row%21];
    img.image = imgTemp;
   // img.bounds = CGRectMake(0, 0, imgTemp.size.width/2, imgTemp.size.height/2);
    UIImage  *desTemp  =  (UIImage*)[_desImgArray objectAtIndex:indexPath.row%21];
...
//    }
    if(indexPath.row % 21 == 7)
    {
        [self sendDataToNextSceneWithPicSubString:@"inax_lt_suolaisi" andPicMount:3 withPageString:@"inax_cheng_lt_page"];
    }
//    if(indexPath.row % 21 == 9)
//    {
//      
//      [self sendDataToNextSceneWithPicSubString:@"inax_lt_hw" andPicMount:2 withPageString:@"inax_lt_page2"];
//
//    }
    if(indexPath.row % 21 == 8)
    {
        
        [self sendDataToNextSceneWithPicSubString:@"inax_lt_oufute" andPicMount:3 withPageString:@"inax_cheng_lt_page"];
...
- (IBAction)rightBtnPressed:(UIButton *)sender {
    ...
    if(scrollViews.contentOffset.x >= 1024/6 * 21*36 - 1024)return;
