...
/*** 若AppaginalTableView不是全屏显示 需修改3个地方
  ** - (id)initWithFrame:(CGRect)frame中定义_scrollView的frame
  ** - (void)layoutPages中_scrollView.contentSize
  ** - (void)layoutPages中page.frame
  **/
...
//        _scrollView = [[APPaginalContainerViewScrollView alloc] initWithFrame:self.bounds];
        _scrollView = [[APPaginalContainerViewScrollView alloc] initWithFrame:(CGRect){.origin=self.bounds.origin, .size=CGSizeMake(1024, self.bounds.size.height)}];
...
//                                         ceilf(_scrollView.bounds.size.height * self.pageSizeRelative.y));
    CGSize pageSizeAbsolute = CGSizeMake(ceilf(1024 * self.pageSizeRelative.x),
                                         ceilf(_scrollView.bounds.size.height * self.pageSizeRelative.y));
...
//    _scrollView.contentSize = CGSizeMake(self.numberOfPages * self.pageSizeAbsolute.width, self.pageSizeAbsolute.height);
    _scrollView.contentSize = CGSizeMake(self.numberOfPages * 1024, self.pageSizeAbsolute.height);
...
//        page.frame = (CGRect){.origin = CGPointMake(self.pageSizeAbsolute.width * i, 0.f), .size = self.pageSizeAbsolute};
        page.frame = (CGRect){.origin = CGPointMake(1024 * i, 0.f), .size = self.pageSizeAbsolute};
    }
}
