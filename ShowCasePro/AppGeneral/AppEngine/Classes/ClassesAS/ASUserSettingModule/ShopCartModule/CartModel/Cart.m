...
{
    // 判断购物车是否已经存在了待添加的商品
    if ([[_cart allKeys] containsObject:item.good.productid]) {
        // 已经存在商品，修改购物车中商品的购买数量
        CartItem *item1 = [_cart objectForKey:item.good.productid];
        ...
        // 不存在该商品，向购物车添加该商品
        [_cart setObject:item forKey:item.good.productid];
        _goodsSize += count;
        item.subQuentity = count;
        [_goodsIDs addObject:item.good.productid];
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:@"GoodsInCartChanged" object:nil];
}
// 删除商品
