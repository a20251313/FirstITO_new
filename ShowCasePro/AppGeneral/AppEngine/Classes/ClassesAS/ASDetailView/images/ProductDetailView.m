...
#import "Tdiyfolder.h"
//#import "ProductVideoViewController.h"
...
@interface ProductDetailView () <UIActionSheetDelegate,ProductIntroViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource,UIScrollViewDelegate>
{
    //产品图片
...
//    //喜欢个数标签
//    __weak IBOutlet UILabel *likeCountLable;
//    
//    //喜欢的个数
//    int likeCount;
    ...
    UILabel *param29;   //伊康佳数量
    UILabel *param30;   //伊康佳实际尺寸
    UILabel *param31;   //伊康佳厚度
    UILabel *param32;   //伊康佳所含内容
...
- (void) reloadData
{
    ...
    relatedProductInfoScrollView.delegate = self;
    ...
    rightViewContentHeight += productName.frame.size.height;
    ...
    productModelnumber.text  = [[self deleteSpaceString:self.tProduct.code] stringByReplacingOccurrencesOfString:@";" withString:@"\n"];
    ...
    NSString *detailText     = [self.tProduct.feature stringByReplacingOccurrencesOfString:@"\\n" withString:@"\n"];
    detailText               = [self deleteSpaceString:detailText];
    ...
    //如果骊住门的规格图片存在  显示
    NSString *standardImagePath = [self deleteSpaceString:self.tProduct.param33];
    ...
    if ([self checkSpaceString:self.tProduct.include_product])
    {
    ...
    rightViewContentHeight = [self layoutParamsLables:param1 string:self.tProduct.param1 orginY:rightViewContentHeight];
    ...
    if (![self checkSpaceString:self.tProduct.pImg])
    {
        ...
        //如果当前是美标  显示美标的 tech icon
        else if ([self.tProduct.brand isEqualToString:@"2"])
        {
            UIImage *techIconImage = [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/%@",kLibrary,self.tProduct.pImg]];
            
            UIImageView *iconImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, techIconImage.size.width/4, techIconImage.size.height/4)];
            iconImageView.image = techIconImage;
            [rightScrollView addSubview:iconImageView];
            
            rightViewContentHeight += 10;
            rightViewContentHeight = [self layoutRightViews:iconImageView orginY:rightViewContentHeight] + 5;
        }
    }
...
- (void) reloadRelatedView
{
    ...
    int pageCount = self.currentRelatedView.frame.size.width / relatedProductInfoScrollView.frame.size.width;
    
    //如果同类产品超过一页  显示page control
    if (pageCount > 1)
    {
        pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        pageControl.center = CGPointMake(relatedProductInfoScrollView.frame.origin.x + relatedProductInfoScrollView.frame.size.width/2, relatedProductInfoScrollView.frame.origin.y + relatedProductInfoScrollView.frame.size.height - 20);
...
- (UIView *) creatMatchProductView
{
    ...
    if ([self.tProduct.type1 isEqualToString:@"42"])////////////铝合金门窗
    {
        NSString *sql = [NSString stringWithFormat:@"select id from tproduct where type2 in (select type2 from tproduct where id = %@ and param31 = 1) and id <> %@ and param31 = 1 order by type1 asc",self.tProduct.productid,self.tProduct.productid];
...
- (UIView *) creatRelatedViewWithImageArray:(NSArray *)array kind:(ProductImageViewKind)kind
{
    ...
    for (int i = 0; i < [array count]; i++)
    {
        ...
        iv.layer.shadowOffset = CGSizeMake(3, 3);
...
- (UIView *) creatRelatedViewWithArray:(NSArray *)array
{
    ...
    for (int i = 0; i < [array count]; i++)
    {
        ...
        ProductIntroView *piv = [[ProductIntroView alloc] initWithFrame:CGRectMake(i*(160 + gap) - page*(gap-1) , 0, 160, 160)];
        piv.productid = array[i];
        piv.delegate = self;
        [piv reloadData];
        
        piv.layer.shadowRadius = 5;
        piv.layer.shadowColor = [UIColor grayColor].CGColor;
        piv.layer.shadowOpacity = 0.8;
        piv.layer.shadowOffset = CGSizeMake(3, 3);
...
{
//    ProductVideoViewController *productVideoViewController = [[ProductVideoViewController alloc] initWithNibName:@"ProductVideoViewController" bundle:nil];
//    [self.navigationController pushViewController:productVideoViewController animated:YES];
}
...
- (void) makeProductImageViewFullScreen:(UITapGestureRecognizer *)sender
{
    ...
    self.fullScreenImageView = [[UIImageView alloc] initWithFrame:self.orginRect];
    self.fullScreenImageView.contentMode = UIViewContentModeScaleAspectFit;
    
    //////判断tag 赋给不同图片
    
    if ((self.selectedImageView.tag >= kProductSizeTag) && (self.selectedImageView.tag < kRoomSceneTag))
    {
        //产品施工图
        self.fullScreenImageView.image = [self.orginProductSizeImage objectAtIndex:(self.selectedImageView.tag - kProductSizeTag)];
    }
    else if ((self.selectedImageView.tag >= kRoomSceneTag) && (self.selectedImageView.tag<510))
    {
        //房间效果图
    ...
    {
        self.fullScreenImageView.frame = CGRectMake(0, 0, 1024, 707);
...
#pragma mark - ProductIntroViewDelegate -

-(void)didSelectProductIntroView:(ProductIntroView *)productIntroView
{
   
    ProductDetailView *pdv = [[ProductDetailView alloc] init];
...
- (void) initSomeStaff
{
    ...
    rightScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(714, 20, 300, 550)];
    ...
    productName.textColor = [UIColor colorWithRed:170./255 green:139./255 blue:75./255 alpha:1];
    ...
    productEnglishName.font = [UIFont boldSystemFontOfSize:13];
    productEnglishName.text = @"产品英文名称";
    productEnglishName.textColor = [UIColor colorWithRed:170./255 green:139./255 blue:75./255 alpha:1];
    ...
    productModelnumberLable.textColor = [UIColor colorWithRed:170./255 green:139./255 blue:75./255 alpha:1];
    productModelnumberLable.font = [UIFont boldSystemFontOfSize:15];
    ...
    productModelnumber.textColor = [UIColor colorWithRed:170./255 green:139./255 blue:75./255 alpha:1];
    productModelnumber.font = [UIFont boldSystemFontOfSize:15];
    ...
    productIntroLable.font = [UIFont boldSystemFontOfSize:15];
    ...
    includeModelLable.font = [UIFont boldSystemFontOfSize:15];
    ...
    ////////按钮相关视图
    buttonContentView = [[UIView alloc] initWithFrame:CGRectMake(18, 490, 600, 32)];
    ...
    param36 = [self creatParamsLablesWithTitle:@"所选款式:"];
}
...
- (UIButton *) creatButtonWithTitle:(NSString *) title andFrame:(CGRect)frame
{
    ...
    [button setTitleColor:[UIColor colorWithRed:170./255 green:139./255 blue:75./255 alpha:1] forState:UIControlStateNormal];
...
- (UIImageView *) spearateImage
{
    ...
    sp.image = [UIImage imageNamed:@"shutiao"];
...
//    [buttonContentView addSubview:spearate];
    ...
    if ([self.tProduct.brand isEqualToString:@"1"] && !self.matchProductView)
    {
        [self addButtonContentWidth:matchProductButton orginWitdh:buttonContentWitdh];
