...
#warning 椭圆路径
    ...
    CGAffineTransform t2 = CGAffineTransformConcat(CGAffineTransformConcat(
                                                                           CGAffineTransformMakeTranslation(-_center_point.x, -_center_point.y),
                                                                           CGAffineTransformMakeScale(1,Circle_Scale)),
                                                   CGAffineTransformMakeTranslation(_center_point.x, _center_point.y));
    ...
    CGPathAddArc(path,
                 ...
                 _center_point.y,
                 _radius,
                 change_cell.current_animation_radian,
                 to_cell.animation_radian,
                 clockwise);
...
- (CGPoint)getPointByRadian:(CGFloat)radian centreOfCircle:(CGPoint)circle_point radiusOfCircle:(CGFloat)circle_radius
{
    ...
    CGFloat c_y = ((cosf(radian) * circle_radius)*Circle_Scale + circle_point.y);
//    CGFloat c_y = (cosf(radian) * circle_radius + circle_point.y);
#warning 修改y值
    
    //NSLog(@"%@:%@",NSStringFromSelector(_cmd),NSStringFromCGPoint(CGPointMake(c_x, c_y)));
...
{
    if([_circle_view_delegate respondsToSelector:@selector(animationBegin)])
    {
        [_circle_view_delegate animationBegin];
    }
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    if([_circle_view_delegate respondsToSelector:@selector(animationEnd)])
    {
        [_circle_view_delegate animationEnd];
    }
}
